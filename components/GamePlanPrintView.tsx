            ...
            <div className="flex justify-between items-start border-b-4 border-slate-900 pb-4 mb-4 flex-wrap gap-4 shrink-0">
                <div className="flex-1 min-w-[400px] flex flex-col">
                    ...
                    <h1 className="text-4xl font-black text-slate-900 uppercase mt-4 mb-2 max-w-[800px] leading-normal pb-1">
                        {teams.find(t => t.id === currentTeamId)?.name || 'MY TEAM'} <span className={gameMode === 'offense' ? "text-red-600" : "text-blue-600"}>{gameMode}</span>
                    </h1>
                    <h2 className="text-2xl font-bold text-slate-500 uppercase max-w-[800px] leading-normal pb-1">
                        {lineups.find(l => l.id === currentLineupId)?.name || 'LINEUP 1'}
                    </h2>
                </div>
                ...
                {rotationsList.map((rot, idx) => (
                    ...
                    `}>
                        <div className="w-36 flex-none flex flex-col items-center justify-center gap-2 border-r border-slate-200 pr-3">
                            <div className="bg-slate-900 text-white w-10 h-10 rounded-lg">
                                <span style={{ display: 'block', width: '40px', height: '40px', textAlign: 'center', fontWeight: 900, fontSize: '18px', paddingTop: '9px', boxSizing: 'border-box' }}>R{rot}</span>
                            </div>
