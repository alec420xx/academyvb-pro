...
import { Users, Pencil, Move, Trash2, Undo, Redo, ChevronRight, UserPlus, X, RefreshCw, Camera, FolderOpen, Plus, Download, Trophy, Shield, Loader2, Hexagon, Layout, FileText, LogIn, LogOut, Copy, Type } from 'lucide-react';
...
export default function App() {
    ...
    const [mode, setMode] = useState<'move' | 'draw' | 'line' | 'arrow' | 'polygon' | 'rect' | 'triangle' | 'text'>('move');
    ...
    const performHitTest = (cx: number, cy: number, width: number, height: number) => {
        ...
        for (let i = paths.length - 1; i >= 0; i--) {
            ...
            if (path.type === 'polygon' || path.type === 'triangle') {
                ...
                const pos = absPoints[0];
                if (pt.x >= pos.x && pt.x <= pos.x + textWidth && pt.y >= pos.y && pt.y <= pos.y + textHeight) hit = true;
            }
    ...
    */
    ...
    const handleCourtDown = (e: any) => {
        ...
        if (mode === 'move') {
            ...
            // Open text input at clicked position
            setTextInputPosition({ x: cx, y: cy });
            setTextInputValue('');
        }
    };
    ...
    }, [isDrawing, selectedShapeIndex, hoveredElement, paths, undo, redo, textInputPosition]);
                            ...
                            <div className="lg:col-span-6 flex flex-col items-center w-full">
                                ...
                                <div className="w-full flex flex-wrap gap-2 justify-between items-center mb-4 lg:mb-6 mt-4 lg:mt-0 px-1 relative z-0 order-2 lg:order-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex items-center bg-slate-800 rounded-lg p-1 border border-slate-700 overflow-x-auto no-scrollbar max-w-[200px] md:max-w-none">
                                            ...
                                            <button onClick={() => setMode('text')} className={`p-1.5 rounded-md ${mode === 'text' ? 'bg-slate-600 text-white' : 'text-slate-400'}`} title="Text"><Type size={18} /></button>
                                        </div>
                                    </div>

                                    {/* Right Side: Colors Only */}
                                    <div className="flex items-center gap-2">
                                        {['draw', 'arrow', 'line', 'rect', 'polygon', 'text'].includes(mode) && (
                                ...
                                <div className="w-full bg-slate-800 p-1 md:p-2 rounded-xl shadow-2xl ring-1 ring-slate-700 relative z-10 order-1 lg:order-2 mt-4 lg:mt-0">
                                    ...
                                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2 z-40">
                                        {['draw', 'line', 'arrow', 'polygon', 'rect', 'triangle', 'text'].includes(mode) && (
